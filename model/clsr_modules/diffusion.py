...
class GaussianDiffusion(nn.Module):
    ...
    def set_new_noise_schedule(self, schedule_opt, device):
        ...
        sampling_timesteps = schedule_opt['sampling_timesteps']
        ddim_sampling_eta = schedule_opt['ddim_sampling_eta']
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, condition_x=None):
        if condition_x is not None:
            ...
            x_recon = self.predict_start_from_noise(
                x, t=t, noise=self.denoise_fn(x, condition_x, t))
        else:
            x_recon = self.predict_start_from_noise(
    ...
    @torch.no_grad()
    def ddim_sample(self, x_in, continous = False):
        device = self.betas.device
        sample_inter = (1 | (self.num_timesteps//10))
        batch = x_in.shape[0]
        shape = (batch, x_in.shape[-3], x_in.shape[-2] * 4, x_in.shape[-1] * 4)
    ...
    def p_sample_loop(self, x_in, continous=False):
        ...
        if not self.conditional:
            ...
            b = shape[0]
            # redefine shape
            # shape = (b, shape[-3], shape[-2] * 8, shape[-1] * 8)
            shape = (b, shape[-3], shape[-2] * 4, shape[-1] * 4)
            img = torch.randn(shape, device=device)
            ret_img = x
            # to make data['SR']
            ret_img = F.interpolate(ret_img, size=shape[2:], mode='bicubic', align_corners=False)
            for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
                img = self.p_sample(img, torch.full(
    ...
    def super_resolution(self, x_in, continous=False):
        if not self.is_ddim_sampling:
            ...
            return self.ddim_sample(x_in, continous)
    ...
    def p_losses(self, x_in, noise=None):
        ...
        if not self.conditional:
            ...
            x_recon = self.denoise_fn(x_noisy, x_in['LR'], t)
